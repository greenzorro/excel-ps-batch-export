...
- **目标文件定位**：
  - 无变换规则：写入Excel文件的第一个sheet（从A2追加，与psd_renderer保持一致）
  - 有变换规则（存在 `.json`）：写入 `_raw.csv` 原始数据文件，系统自动调用 transform.py 处理
- **安全数据写入**：保留表头行（第1行），从A2单元格开始追加数据
...
- **错误处理**：单张图片错误不影响其他图片，提供详细报告；任一任务失败或没有可执行任务时，批量命令返回非零退出码，供剪贴板导入器和自动化流程准确判定结果
...
- **可见性变量**：`@水印#v`（显示/隐藏控制）
- **段落文本**：`@描述#t_p`（自动换行段落）
- **段落垂直对齐**：`@描述#t_p_pm`（段落 + 垂直居中）、`@描述#t_c_p_pb`（居中段落 + 垂直底部）

**对齐参数：**
- `_c`：水平居中
- `_r`：水平右对齐
- `_p`：段落文本（自动换行）
- `_pm`：段落内垂直居中（与 `_p` 组合使用，如 `#t_p_pm`）
- `_pb`：段落内垂直底部（与 `_p` 组合使用，如 `#t_p_pb`）
