...
@pytest.fixture
def text_render_objects():
    """创建用于文本位置计算的对象"""
    test_img = Image.new("RGB", (500, 100))
    draw = ImageDraw.Draw(test_img)
    font = ImageFont.load_default()
    return {"draw": draw, "font": font}
...
class TestPreciseTextPosition:
    ...
    def test_precise_center_alignment_calculation(self, text_render_objects):
        """测试精确的居中对齐计算"""
        text = "Test"
        layer_width = 100
        font_size = 16
        ...
        # 验证X位置在合理范围（增加容差以适应不同字体）
        assert abs(x_pos - expected_x) < 10, f"X位置应该在预期附近: 期望≈{expected_x}, 实际{x_pos}"
        # 验证位置确实在中心附近
        center_point = layer_width / 2
        assert abs(x_pos - center_point) < expected_text_width / 2 + 10, "应该在中心点附近"
        # 验证Y位置是负数
        assert y_pos < 0, f"Y位置应该是负数: 实际{y_pos}"

    def test_precise_left_alignment_calculation(self, text_render_objects):
        """测试精确的左对齐计算"""
        text = "ABC"
        layer_width = 150
        font_size = 14
        ...
        # 验证X位置在预期附近
        assert abs(x_pos - expected_x) < 1, f"左对齐X位置应该在0附近: 期望≈{expected_x}, 实际{x_pos}"
        # 验证Y位置是负数
        assert y_pos < 0, f"Y位置应该是负数: 实际{y_pos}"

    def test_mixed_text_precise_calculation(self, text_render_objects):
        """测试中英文混合文本的精确计算"""
        text = "中文ABC"
        layer_width = 200
        font_size = 18
    ...
    def test_alignment_comparison(self, text_render_objects):
        """测试不同对齐方式的相对位置关系"""
        text = "Sample"
        layer_width = 120
        font_size = 15
...
class TestImprovedTextPositionAssertions:
    ...
    def test_assertion_precision_improvement(self, text_render_objects):
        ...
        for text, width, size, alignment in test_cases:
            ...
            # 新算法使用真实字体度量，验证基本逻辑而不是具体数值
            # 验证Y位置是负数（文本基线偏移）
            assert y_pos < 0, f"{text} ({alignment}) Y位置应该是负数: 实际{y_pos}"
